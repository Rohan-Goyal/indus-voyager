...
def assist_to_planet(
    ...
    planet_orbit = Ephem.from_body(
        planet, time_range(launch_date, end=target_date + TimeDelta(1 * u.yr))
    )
    ...
    flyby_end = flyby.propagate(target_date + TimeDelta(0 * u.wk))
...
jupiter_end, jupiter_cost = assist_to_planet(
    Jupiter, jupiter_date, init_orbit_end, anom=(0, 150)
)
saturn_end, saturn_cost = assist_to_planet(
    Saturn, saturn_date, jupiter_end, color="blue", anom=(30, 80)
)
uranus_end, uranus_cost = assist_to_planet(
    Uranus, uranus_date, saturn_end, color="red", anom=(5, 70)
)
neptune_end, neptune_cost = assist_to_planet(
    Neptune, neptune_date, uranus_end, color="green", anom=(40, 70)
)
